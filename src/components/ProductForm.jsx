...
export default function ProductForm({onAdd}) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-3xl shadow-xl mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <input type="text" placeholder="Categoria" value={categoria} onChange={(e) => setCategoria(e.target.value)}
          className="border p-4  rounded-2xl"/>
        <input
          type="number" placeholder="Preço" value={preco} onChange={(e) => setPreco(e.target.value)}
          className="border p-4 rounded-2xl"/>
